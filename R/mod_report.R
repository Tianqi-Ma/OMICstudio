...
mod_report_ui <- function(id) {
  ...
  controls <- shiny::tagList(
    ...
    shiny::textInput(ns("title"), NULL, value = "OMICstudio analysis report"),
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    build_rmd <- function(title, entries) {
      lines <- c(
        ...
        "---",
        "",
        "> Generated by OMICstudio from the reproducibility log.",
        "")
    ...
    build_html <- function(title, entries) {
      ...
      head <- c(
        ...
        sprintf("<p class='muted'>Generated by OMICstudio &middot; %s</p>",
    ...
    output$summary <- shiny::renderUI({
      ...
      shiny::tagList(
        ...
        shiny::tags$ol(class = "omicstudio-steps",
          lapply(entries, function(e) {
            shiny::tags$li(shiny::tags$b(e$step),
                           shiny::tags$span(class = "omicstudio-muted",
                                            paste0("  (", e$time, ")")))
          }))
      )
    })
    ...
    output$download_report <- shiny::downloadHandler(
      filename = function()
        paste0("omicstudio_report_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".html"),
      content = function(file) {
        title   <- if (nzchar(trimws(input$title %||% ""))) input$title
                   else "OMICstudio analysis report"
